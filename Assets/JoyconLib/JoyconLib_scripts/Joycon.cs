...
public class Joycon
{
    ...
    {
        stop_polling = true;
        PrintArray(max, format: "Max {0:ss}", d: DebugType.IMU);
        PrintArray(sum, format: "Sum {0:ss}", d: DebugType.IMU);
    ...
    public void Update()
    {
        if (state > state_.NO_JOYCONS)
        {
            ...
            while (reports.Count > 0)
            {
                ...
                ts_de = report_buf[1];
                DebugPrint(string.Format("Dequeue. Queue length: {0:d}. Packet ID: {1:X2}. Timestamp: {2:X2}. Lag to dequeue: {3:ss}. Lag between packets (expect 15ms): {4:ss}",
    ...
    private void SendRumble(byte[] buf)
    {
        ...
        PrintArray(buf_, DebugType.RUMBLE, format: "Rumble data sent: {0:ss}");
    ...
    private byte[] Subcommand(byte sc, byte[] buf, uint len, bool print = true)
    {
        ...
        if (print) { PrintArray(buf_, DebugType.COMMS, len, 11, "Subcommand 0x" + string.Format("{0:X2}", sc) + " sent. Data: 0x{0:ss}"); };
        ...
        else if (print) { PrintArray(response, DebugType.COMMS, report_len - 1, 1, "Response ID 0x" + string.Format("{0:X2}", response[0]) + ". Data: 0x{0:ss}"); }
        return response;
    }
    private void dump_calibration_data()
    {
        ...
        PrintArray(stick_cal, len: 6, start: 0, format: "Stick calibration data: {0:ss}");
        ...
        PrintArray(gyr_neutral, len: 3, d: DebugType.IMU, format: "User gyro neutral position: {0:ss}");
        ...
        if (gyr_neutral[0] + gyr_neutral[1] + gyr_neutral[2] == -3 || Math.Abs(gyr_neutral[0]) > 100 || Math.Abs(gyr_neutral[1]) > 100 || Math.Abs(gyr_neutral[2]) > 100)
        {
            ...
            PrintArray(gyr_neutral, len: 3, d: DebugType.IMU, format: "Factory gyro neutral position: {0:ss}");
        }
    }
    ...
    private void PrintArray<T>(T[] arr, DebugType d = DebugType.NONE, uint len = 0, uint start = 0, string format = "{0:ss}")
    {
